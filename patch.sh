...
# Copyright:: Copyright 2023, Cinc Project
...
rm -rf chef-foundation
git config --global user.email || git config --global user.email "maintainers@cinc.sh"
echo "Cloning ${REF:-main} branch from ${ORIGIN:-https://github.com/chef/chef-foundation.git}"
git clone -q -b ${REF:-main} ${ORIGIN:-https://github.com/chef/chef-foundation.git}
cd chef-foundation
git_patch chef-foundation ${CINC_REF}
cd $TOP_DIR
